        ...
        <div className="w-10/12 mx-auto px-6">
            <div className=" border-gray-200 border-b-2">
                ...
                <div className="flex gap-8 mb-8 overflow-hidden">
                    {
                        dishesList.map((dish) => <CarouselItem key={dish.id}  dish = {dish} />)
                    }
                </div>
            </div>
            <div>
                ...
                <div className="flex gap-14 overflow-hidden">
                    {
                        topResturantList.map((resturant) => <Link key={resturant.info.id} to={`/resturantDetails/${resturant.info.id}`}> <ResturantChain resData={resturant.info} /></Link>)
                    }
                </div>
            </div>
            ...
            <div className="flex justify-between">
                ...
                <div>
                    ...
                    <button
                        ...
                        className="hover:bg-custom-defaultLight bg-custom-default text-white font-bold py-1 px-2 rounded transition-all ease-in-out">Search</button>
                </div>
                {/* <input
            ...
            <div className="flex flex-wrap gap-16 justify-stretch">
                {filteredResturant.map((restaurant) => (
                    ...
                    >
                        {
                           ( restaurant.info.isOpen) ? <ResturantCard resData={restaurant.info} /> : <ResturantCardClosed resData={restaurant.info} /> 
                        }
